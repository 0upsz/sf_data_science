def middle_predict(number:int=1) -> int:
    ...
    Args:
        number (int, optional): загадываемое число. По умолчанию 1.
    ...
    middle = (a+b)//2 #середина диапазона, округляется до целого значения до запятой
...
def score_game(middle_predict) -> int:
    ...
    count_ls = [] #список, в который будут количество попыток угадывания
    import numpy as np
    np.random.seed(1)
